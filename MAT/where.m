function [s, swapbyteorder fileversion]=where(filename, varargin)
...
if nargin==2 && isnumeric(varargin{1})
    [fh, swapbyteorder FileLevel info]=MATOpen(filename,'r');
    if fh<0
        return
    end
...
fileversion=str2double(info(8:10));
...
for variable=1:NumberOfVar
    ...
    %fprintf('Found %s at %d\n', Name, TOffset);
    % If where(filename, tagoffset) call type, s(1).name will be empty
    if strcmp(Name,'0123456789')
        s(variable).name='$UNKNOWN$';
        s(variable).TagOffset=TOffset;
        s(variable).class=VClass;
    end
...
for i=1:length(s)
    ...
    % modify 26.07.08
    if isfield(s,'DataOffset') && iscell(s(i).DataOffset)
