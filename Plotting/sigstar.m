...
%remove values that are the same group
diffGroups = cellfun(@(x) length(unique(x)) > 1,groups);
groups = groups(diffGroups);
stats = stats(diffGroups);
...
H(2)=text(mean(x(:)),mean(y)+myRange(ylim)*offset,stars,...
    ...
    'BackGroundColor','none','FontSize',20);
